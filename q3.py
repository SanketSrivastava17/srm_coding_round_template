...
def find_overloaded_users(events):
    ...
    for user_id, timestamps in user_events.items():
        ...
        for i in range(len(timestamps)):
            for j in range(i + 2, len(timestamps)):
                
                if timestamps[j] - timestamps[i] < 10:
                    overloaded.add(user_id)
                    break
            if user_id in overloaded:
                break
...
if __name__ == "__main__":
    ...
    print(find_overloaded_users(events)) 

    print(find_overloaded_users([])) 

    print(find_overloaded_users([(1, 1), (1, 20), (1, 40)]))  
